...
//                return $row->status ? Team::STATUS_SELECT[$row->status] : '';
    ...
    public function showOnsiteTeams(Request $request)
    {
        if ($request->ajax()) {
            ...
            $table->editColumn('team_leader.phone_number', function ($row) {
                return $row->team_leader ? (is_string($row->team_leader) ? $row->team_leader : $row->team_leader->phone_number) : '';
            });
...
//            $condition_id = ParticipationMethod::where('title', 'Virtual')->pluck('id')->first();
            $query = Team::with(['team_leader', 'challenge', 'actual_solution', 'mentorship_needed', 'participation_method'])->select(sprintf('%s.*', (new Team)->table))->where('status' , '!=', 'accepted_onsite')->orWhere('status', NULL);
            $table = Datatables::of($query);
