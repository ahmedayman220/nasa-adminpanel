    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.qr-codes.update", [$qrCode->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('bootcamp_participant') ? 'is-invalid' : '' }}" name="bootcamp_participant_id" id="bootcamp_participant_id" required>
                    @foreach($bootcamp_participants as $id => $entry)
                        <option value="{{ $id }}" {{ (old('bootcamp_participant_id') ? old('bootcamp_participant_id') : $qrCode->bootcamp_participant->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('qr_code_value') ? 'is-invalid' : '' }}" type="text" name="qr_code_value" id="qr_code_value" value="{{ old('qr_code_value', $qrCode->qr_code_value) }}" required>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\QrCode::STATUS_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('status') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="status_{{ $key }}" name="status" value="{{ $key }}" {{ old('status', $qrCode->status) === (string) $key ? 'checked' : '' }}>
