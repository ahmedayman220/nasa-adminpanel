...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.participantWorkshopAssignment.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.participant-workshop-assignments.update", [$participantWorkshopAssignment->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('bootcamp_participant') ? 'is-invalid' : '' }}" name="bootcamp_participant_id" id="bootcamp_participant_id" required>
                    @foreach($bootcamp_participants as $id => $entry)
                        <option value="{{ $id }}" {{ (old('bootcamp_participant_id') ? old('bootcamp_participant_id') : $participantWorkshopAssignment->bootcamp_participant->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('workshop_schedule') ? 'is-invalid' : '' }}" name="workshop_schedule_id" id="workshop_schedule_id" required>
                    @foreach($workshop_schedules as $id => $entry)
                        <option value="{{ $id }}" {{ (old('workshop_schedule_id') ? old('workshop_schedule_id') : $participantWorkshopAssignment->workshop_schedule->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('attendance_status') ? 'is-invalid' : '' }}" name="attendance_status" id="attendance_status" required>
                    ...
                    @foreach(App\Models\ParticipantWorkshopAssignment::ATTENDANCE_STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('attendance_status', $participantWorkshopAssignment->attendance_status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control timepicker {{ $errors->has('check_in_time') ? 'is-invalid' : '' }}" type="text" name="check_in_time" id="check_in_time" value="{{ old('check_in_time', $participantWorkshopAssignment->check_in_time) }}" required>
