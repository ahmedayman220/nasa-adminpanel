...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.skills.update", [$skill->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="category">{{ trans('cruds.skill.fields.category') }}</label>
                <input class="form-control {{ $errors->has('category') ? 'is-invalid' : '' }}" type="text" name="category" id="category" value="{{ old('category', $skill->category) }}">
                @if($errors->has('category'))
                    <div class="invalid-feedback">
                        {{ $errors->first('category') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.skill.fields.category_helper') }}</span>
            </div>
            <div class="form-group">
