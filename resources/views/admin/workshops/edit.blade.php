...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.workshops.update", [$workshop->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="descriptions">{{ trans('cruds.workshop.fields.descriptions') }}</label>
                <textarea class="form-control ckeditor {{ $errors->has('descriptions') ? 'is-invalid' : '' }}" name="descriptions" id="descriptions">{!! old('descriptions', $workshop->descriptions) !!}</textarea>
                @if($errors->has('descriptions'))
                    <div class="invalid-feedback">
                        {{ $errors->first('descriptions') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.workshop.fields.descriptions_helper') }}</span>
            </div>
            <div class="form-group">
          ...
          return loader.file
            .then(function (file) {
              return new Promise(function(resolve, reject) {
                ...
                xhr.open('POST', '{{ route('admin.workshops.storeCKEditorImages') }}', true);
