...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.bootcampConfirmation.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.bootcamp-confirmations.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('national') ? 'is-invalid' : '' }}" name="national_id" id="national_id" required>
                    @foreach($nationals as $id => $entry)
                        <option value="{{ $id }}" {{ old('national_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('slot') ? 'is-invalid' : '' }}" name="slot_id" id="slot_id">
                    @foreach($slots as $id => $entry)
                        <option value="{{ $id }}" {{ old('slot_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('slot'))
                    <div class="invalid-feedback">
