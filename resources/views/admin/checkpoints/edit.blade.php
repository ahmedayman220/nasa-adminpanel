    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.checkpoints.update", [$checkpoint->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('event') ? 'is-invalid' : '' }}" name="event_id" id="event_id" required>
                    @foreach($events as $id => $entry)
                        <option value="{{ $id }}" {{ (old('event_id') ? old('event_id') : $checkpoint->event->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('event'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('checkpoint_type') ? 'is-invalid' : '' }}" name="checkpoint_type_id" id="checkpoint_type_id">
                    @foreach($checkpoint_types as $id => $entry)
                        <option value="{{ $id }}" {{ (old('checkpoint_type_id') ? old('checkpoint_type_id') : $checkpoint->checkpoint_type->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description">{!! old('description', $checkpoint->description) !!}</textarea>
