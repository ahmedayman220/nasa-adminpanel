...
@can('evaluation_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            ...
            @include('csvImport.modal', ['model' => 'Evaluation', 'route' => 'admin.evaluations.parseCsvImport'])
        </div>
    </div>
@endcan
<div class="card">
    ...
    <div class="card-body">
        <table class=" table table-bordered table-striped table-hover ajaxTable datatable datatable-Evaluation">
            <thead>
                <tr>
                    <th width="10">
                ...
                <tr>
                    ...
                    <td>
                        <select class="search">
                            <option value>{{ trans('global.all') }}</option>
                            @foreach($evaluation_criteria as $key => $item)
                                <option value="{{ $item->name }}">{{ $item->name }}</option>
                            @endforeach
                        </select>
                    </td>
...
{ data: 'actions', name: '{{ trans('global.actions') }}' }
  ...
  let table = $('.datatable-Evaluation').DataTable(dtOverrideGlobals);
