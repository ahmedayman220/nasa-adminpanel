...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.memberCheckpoint.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.member-checkpoints.update", [$memberCheckpoint->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('member') ? 'is-invalid' : '' }}" name="member_id" id="member_id" required>
                    @foreach($members as $id => $entry)
                        <option value="{{ $id }}" {{ (old('member_id') ? old('member_id') : $memberCheckpoint->member->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('checkpoint') ? 'is-invalid' : '' }}" name="checkpoint_id" id="checkpoint_id" required>
                    @foreach($checkpoints as $id => $entry)
                        <option value="{{ $id }}" {{ (old('checkpoint_id') ? old('checkpoint_id') : $memberCheckpoint->checkpoint->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('completed') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="completed" id="completed" value="1" {{ $memberCheckpoint->completed || old('completed', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('completion_time') ? 'is-invalid' : '' }}" type="text" name="completion_time" id="completion_time" value="{{ old('completion_time', $memberCheckpoint->completion_time) }}">
