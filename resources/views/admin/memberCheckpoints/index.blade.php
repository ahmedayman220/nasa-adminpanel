    ...
    @can('member_checkpoint_create')
        <div style="margin-bottom: 10px;" class="row">
            <div class="col-lg-12">
                ...
                @include('csvImport.modal', ['model' => 'MemberCheckpoint', 'route' => 'admin.member-checkpoints.parseCsvImport'])
            </div>
        </div>
    @endcan
    <div class="card">
        ...
        <div class="card-body">
            <table
                class=" table table-bordered table-striped table-hover ajaxTable datatable datatable-MemberCheckpoint">
                <thead>
                <tr>
                    <th width="10">
                ...
                <tr>
                    ...
                    <td>
                        <select class="search">
                            <option value>{{ trans('global.all') }}</option>
                            @foreach($checkpoints as $key => $item)
                                <option value="{{ $item->name }}">{{ $item->name }}</option>
                            @endforeach
                        </select>
                    </td>
    ...
    <script>
        $(function () {
            ...
            let dtOverrideGlobals = {
                ...
                retrieve: true,
                aaSorting: [],
                ajax: "{{ route('admin.member-checkpoints.index') }}",
                columns: [
                    {data: 'placeholder', name: 'placeholder'},
                    {data: 'id', name: 'id'},
                    {data: 'member_uuid', name: 'member.uuid'},
                    {data: 'member_name', name: 'member.name'},
                    {data: 'checkpoint_name', name: 'checkpoint.name'},
            ...
            let table = $('.datatable-MemberCheckpoint').DataTable(dtOverrideGlobals);
