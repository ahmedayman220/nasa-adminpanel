...
@section('content')
    ...
    <div class="card">
        ...
        <div class="d-flex">
            {{-- Start Manual Div --}}
            <div class="card-body">
                <form method="POST" action="{{ route("admin.bootcamp-attendees.store") }}"
                      enctype="multipart/form-data">
                    @csrf
                    <div class="form-group">
                        <label class="required"
                               for="bootcamp_participant_id">{{"Update Attendee Status With UUID" }}</label>
                        <select
                            class="form-control select2 {{ $errors->has('bootcamp_participant') ? 'is-invalid' : '' }}"
                            name="bootcamp_participant_id" id="bootcamp_participant_id" required>
                            @foreach($bootcamp_participants as $participant)
                                <option
                                    value="{{ $participant->id }}" {{ old('bootcamp_participant_id') == $participant->id ? 'selected' : '' }}>{{ $participant->uuid." : ".$participant->name_en}}</option>
                            @endforeach
                        </select>
                        ...
                        @endif
                        <span
                            class="help-block">{{ trans('cruds.bootcampAttendee.fields.bootcamp_participant_helper') }}</span>
                    </div>
                    <div class="form-group">
            ...
            <div class="card-body">
                <form method="POST" action="{{ route("admin.bootcamp-attendees.store") }}"
                    ...
                    <div class="form-group">
                        <label class="required"
                               for="bootcamp_participant_id">{{"Update Attendee Status With National" }}</label>
                        <select
                            class="form-control select2 {{ $errors->has('bootcamp_participant') ? 'is-invalid' : '' }}"
                            name="bootcamp_participant_id" id="bootcamp_participant_id" required>
                            @foreach($bootcamp_participants as $participant)
                                <option
                                    value="{{ $participant->id }}" {{ old('bootcamp_participant_id') == $participant->national ? 'selected' : '' }}>{{ $participant->national}}
        ...
        <div class="card-body">
            <table
                class=" table table-bordered table-striped table-hover ajaxTable datatable datatable-BootcampAttendee">
                <thead>
                <tr>
                    <th width="10">
        ...
        $(function () {
            ...
            let dtOverrideGlobals = {
                ...
                columns: [
                    ...
                    { data: 'bootcamp_participant_national', name: 'bootcamp_participant.national' },
                    { data: 'bootcamp_participant_email', name: 'bootcamp_participant.email' },
