...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.bootcampAttendee.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.bootcamp-attendees.update", [$bootcampAttendee->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('bootcamp_details') ? 'is-invalid' : '' }}" name="bootcamp_details_id" id="bootcamp_details_id" required>
                    @foreach($bootcamp_details as $id => $entry)
                        <option value="{{ $id }}" {{ (old('bootcamp_details_id') ? old('bootcamp_details_id') : $bootcampAttendee->bootcamp_details->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('bootcamp_participant') ? 'is-invalid' : '' }}" name="bootcamp_participant_id" id="bootcamp_participant_id" required>
                    @foreach($bootcamp_participants as $id => $entry)
                        <option value="{{ $id }}" {{ (old('bootcamp_participant_id') ? old('bootcamp_participant_id') : $bootcampAttendee->bootcamp_participant->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\BootcampAttendee::CATEGORY_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('category') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="category_{{ $key }}" name="category" value="{{ $key }}" {{ old('category', $bootcampAttendee->category) === (string) $key ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\BootcampAttendee::ATTENDANCE_STATUS_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('attendance_status') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="attendance_status_{{ $key }}" name="attendance_status" value="{{ $key }}" {{ old('attendance_status', $bootcampAttendee->attendance_status) === (string) $key ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('check_in_time') ? 'is-invalid' : '' }}" type="text" name="check_in_time" id="check_in_time" value="{{ old('check_in_time', $bootcampAttendee->check_in_time) }}">
