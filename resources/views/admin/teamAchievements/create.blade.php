...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.teamAchievement.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.team-achievements.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('achievement') ? 'is-invalid' : '' }}" name="achievement_id" id="achievement_id">
                    @foreach($achievements as $id => $entry)
                        <option value="{{ $id }}" {{ old('achievement_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('earned_at') ? 'is-invalid' : '' }}" type="text" name="earned_at" id="earned_at" value="{{ old('earned_at') }}">
