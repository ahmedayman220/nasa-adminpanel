...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.workshopSchedule.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.workshop-schedules.update", [$workshopSchedule->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('workshop') ? 'is-invalid' : '' }}" name="workshop_id" id="workshop_id" required>
                    @foreach($workshops as $id => $entry)
                        <option value="{{ $id }}" {{ (old('workshop_id') ? old('workshop_id') : $workshopSchedule->workshop->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('schedule_time') ? 'is-invalid' : '' }}" type="text" name="schedule_time" id="schedule_time" value="{{ old('schedule_time', $workshopSchedule->schedule_time) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('capacity') ? 'is-invalid' : '' }}" type="number" name="capacity" id="capacity" value="{{ old('capacity', $workshopSchedule->capacity) }}" step="1" required>
