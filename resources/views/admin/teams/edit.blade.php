    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.teams.update", [$team->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('uuid') ? 'is-invalid' : '' }}" type="text" name="uuid" id="uuid" value="{{ old('uuid', $team->uuid) }}" required>
                @if($errors->has('uuid'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('team_leader') ? 'is-invalid' : '' }}" name="team_leader_id" id="team_leader_id" required>
                    @foreach($team_leaders as $id => $entry)
                        <option value="{{ $id }}" {{ (old('team_leader_id') ? old('team_leader_id') : $team->team_leader->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('team_name') ? 'is-invalid' : '' }}" type="text" name="team_name" id="team_name" value="{{ old('team_name', $team->team_name) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('challenge') ? 'is-invalid' : '' }}" name="challenge_id" id="challenge_id" required>
                    @foreach($challenges as $id => $entry)
                        <option value="{{ $id }}" {{ (old('challenge_id') ? old('challenge_id') : $team->challenge->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('actual_solution') ? 'is-invalid' : '' }}" name="actual_solution_id" id="actual_solution_id" required>
                    @foreach($actual_solutions as $id => $entry)
                        <option value="{{ $id }}" {{ (old('actual_solution_id') ? old('actual_solution_id') : $team->actual_solution->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('mentorship_needed') ? 'is-invalid' : '' }}" name="mentorship_needed_id" id="mentorship_needed_id" required>
                    @foreach($mentorship_neededs as $id => $entry)
                        <option value="{{ $id }}" {{ (old('mentorship_needed_id') ? old('mentorship_needed_id') : $team->mentorship_needed->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('participation_method') ? 'is-invalid' : '' }}" name="participation_method_id" id="participation_method_id" required>
                    @foreach($participation_methods as $id => $entry)
                        <option value="{{ $id }}" {{ (old('participation_method_id') ? old('participation_method_id') : $team->participation_method->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('limited_capacity') ? 'is-invalid' : '' }}">
                    <input class="form-check-input" type="checkbox" name="limited_capacity" id="limited_capacity" value="1" {{ $team->limited_capacity || old('limited_capacity', 0) === 1 ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('members_participated_before') ? 'is-invalid' : '' }}">
                    <input class="form-check-input" type="checkbox" name="members_participated_before" id="members_participated_before" value="1" {{ $team->members_participated_before || old('members_participated_before', 0) === 1 ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('project_proposal_url') ? 'is-invalid' : '' }}" type="text" name="project_proposal_url" id="project_proposal_url" value="{{ old('project_proposal_url', $team->project_proposal_url) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('project_video_url') ? 'is-invalid' : '' }}" type="text" name="project_video_url" id="project_video_url" value="{{ old('project_video_url', $team->project_video_url) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('team_rating') ? 'is-invalid' : '' }}" type="number" name="team_rating" id="team_rating" value="{{ old('team_rating', $team->team_rating) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('total_score') ? 'is-invalid' : '' }}" type="number" name="total_score" id="total_score" value="{{ old('total_score', $team->total_score) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status">
                    ...
                    @foreach(App\Models\Team::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', $team->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('submission_date') ? 'is-invalid' : '' }}" type="text" name="submission_date" id="submission_date" value="{{ old('submission_date', $team->submission_date) }}">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('nots') ? 'is-invalid' : '' }}" name="nots" id="nots">{!! old('nots', $team->nots) !!}</textarea>
                @if($errors->has('nots'))
                    <div class="invalid-feedback">
                        {{ $errors->first('nots') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('extra_field') ? 'is-invalid' : '' }}" type="text" name="extra_field" id="extra_field" value="{{ old('extra_field', $team->extra_field) }}">
