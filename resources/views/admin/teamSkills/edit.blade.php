...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.teamSkill.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.team-skills.update", [$teamSkill->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('skill') ? 'is-invalid' : '' }}" name="skill_id" id="skill_id" required>
                    @foreach($skills as $id => $entry)
                        <option value="{{ $id }}" {{ (old('skill_id') ? old('skill_id') : $teamSkill->skill->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('skill'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('proficiency_level') ? 'is-invalid' : '' }}" type="number" name="proficiency_level" id="proficiency_level" value="{{ old('proficiency_level', $teamSkill->proficiency_level) }}" step="0.01" required>
